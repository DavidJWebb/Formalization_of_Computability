...
/- ϕₑ,ₛ(_). Some sources use s+1 instead of s here, but we follow Soare.
In this convention, no program halts in 0 steps -/
...
def W_enum_prefix (e : ℕ) : ℕ → List ℕ
| 0     => W_stage_new e 0
| s + 1 =>
  let prev := W_enum_prefix e s
