...
lemma ϕNew_disjoint_gt (h : s > t) : Disjoint (ϕNew e s) (ϕNew e t) := by
  ...
  simp_rw [ϕNew_mem]
  intro a ⟨_, has1⟩ ⟨hat, _⟩
  exact Ne.elim has1 (ϕ_halts_mono (le_sub_one_of_lt h) hat)

lemma ϕNew_pairwise_disjoint (h : s ≠ t) : Disjoint (ϕNew e s) (ϕNew e t) := by
  replace h := Nat.lt_or_gt_of_ne h
  apply Or.elim h
  <;> intro h1
  · exact List.Disjoint.symm (ϕNew_disjoint_gt h1)
  · exact ϕNew_disjoint_gt h1

/- It is sometimes useful to work with W_{e,s} instead of W_{e, s+1} -/
lemma Ws_eq : Ws e s = (Ws e (s-1)) ++ (ϕNew e s) := by
  cases s with | zero => simp [ϕNew] | succ s => simp [Ws]

/- W_e can be created as a disjoint union of new elements-/
lemma We_eq_union_ϕNew : W e = ⋃ s, (ϕNew e s).toFinset := by
  rw [W_eq_union_Ws]
  apply subset_antisymm
  <;> intro x h
  <;> simp_all only [Set.mem_iUnion, Finset.mem_coe, mem_toFinset]
  <;> replace ⟨s, h⟩ := h
  · replace ⟨r, h⟩ := Ws_runtime h
    exact ⟨r, ((ϕNew_runtime e x r).mpr h.right.left)⟩
  · exact ⟨s, Ws_mem.mpr (ϕNew_mem.mp h).left⟩
...
/- If all large enough ϕNew are empty, then W e is finite. -/
lemma ϕNew_stabilizes_We_finite (h : ∀ t > s, ϕNew e t = ∅) :
    (W e).Finite := by
  ...
  · simp only [Finset.finite_toSet, implies_true, Finset.coe_nonempty, true_and]
    ...
    refine Set.setOf_subset_setOf_of_imp ?_
    simp only [toFinset_nonempty_iff, ne_eq]
    exact fun a a_1 ↦ Nat.le_of_not_lt fun a_2 ↦ a_1 (h a a_2)
  · simp only [Finset.disjoint_coe, disjoint_toFinset_iff_disjoint]
    intro i j h1
    exact ϕNew_pairwise_disjoint h1

/- If W e is finite, then all large enough ϕNew are empty. -/
lemma We_finite_ϕNew_stabilizes (h : (W e).Finite) :
    ∃ s, ∀ t > s, ϕNew e t = ∅ := by
  ...
  obtain ⟨P, h1⟩ := Set.Finite.exists_finset h
  · by_cases h2 : P = ∅
    · use 0
      simp_all
  ...
  · simp only [Finset.disjoint_coe]
    intro i j h1
    simp only [disjoint_toFinset_iff_disjoint]
    exact ϕNew_pairwise_disjoint h1

/- W e is finite iff all large enough ϕNew are empty. -/
lemma We_finite_iff_ϕNew_stabilizes : (W e).Finite ↔ (∃ t, ∀ s > t, ϕNew e s = ∅) := by
  constructor
  · exact We_finite_ϕNew_stabilizes
  · intro ⟨_, h⟩
    exact ϕNew_stabilizes_We_finite h
...
def enter_queue (e : ℕ) : ℕ → List ℕ
  | 0 => []
  | s + 1 => (enter_queue e s).tail ++ (ϕNew e (s+1))

open Primrec

lemma enter_queue_primrec (e : ℕ) : Primrec (enter_queue e) := by
  have hstep : Primrec₂ (fun (s : ℕ) (L : List ℕ) => L.tail ++ ϕNew e (s + 1)) := by
    refine Primrec.list_append.comp₂ ?_ ?_
    · exact ((Primrec.list_tail : Primrec (fun L : List ℕ => L.tail)).comp Primrec₂.right)
    · exact (ϕNew_primrec e).comp (Primrec.succ.comp Primrec₂.left)
  refine (Primrec.nat_rec₁ ([] : List ℕ) hstep).of_eq ?_
  intro s
  induction s with
  | zero => rfl
  | succ s ih => simp [enter_queue, ih]

/- If n is in the queue at stage s, then ϕ_{e, s}(n)↓ -/
lemma enter_queue_halts (h : n ∈ enter_queue e s) : ϕs_halts e s n := by
  induction s with | zero | succ s ih
  · tauto
  · simp only [enter_queue, List.mem_append] at h
    cases h with | inl h | inr h
    · exact ϕ_halts_mono (Nat.le_add_right s 1) (ih (List.mem_of_mem_tail h))
    · simp_all only [ϕs_halts, ϕNew_mem, add_tsub_cancel_right, Bool.not_eq_true,
...
lemma enter_queue_mem (e n : ℕ) : (∃ s, n ∈ enter_queue e s) ↔ ϕ_halts e n := by
  ...
  · intro ⟨s, h⟩
    ...
    use s
  · intro h
    simp only [ϕ_halts_runtime_exists, ← ϕNew_runtime] at h
    obtain ⟨r, h⟩ := h
    use r
    unfold enter_queue
...
lemma enter_queue_ϕNew (e : ℕ) (h : s ≥ t) :
    (enter_queue e s) ⊆
    (enter_queue e t) ++ flatten ((range (s - t)).map (λ i ↦ ϕNew e (t + i + 1))) := by
...
lemma enter_queue_exit_exact (h : List.idxOf? n (enter_queue e s) = some k) :
  ...
  replace ⟨r, h2⟩ := runtime_min' h1
  have h3 : r ≤ s := h2.right
  ...
  · constructor
    ...
    · exact h4 0 ((ϕNew_runtime e n r).mpr h2.left)
  · constructor
    · contrapose ih
      exact mem_of_mem_tail ih
    · rw [← ϕNew_runtime] at h2
      have h5 := (h4 (t+1)) h2.left
...
instance Wenum_primrec : Primrec (Wenum e)  := by
...
lemma queue_depletes (h : (W e).Finite) :
  ...
  have hs1 := enter_queue_ϕNew e hs
  have hi1 : ∀ i, ϕNew e (t+i+1) = [] := by
    intro i
    apply h
    omega
...
lemma Ws_stabilizes_implies_We_eq_Ws (h : ∃ t, ∀ s ≥ t, Ws e s = Ws e t) :
  ...
  <;> intro h1
  · obtain ⟨s, h1⟩ := h1
    simp only [coe_toFinset, Set.mem_setOf_eq]
    by_cases hts : s > t
    · replace hts := le_of_succ_le hts
      simp_all only [ge_iff_le, Std.le_refl, Ws_mem]
    · push Not at hts
      exact IsPrefix.mem h1 ((Ws_mono e) hts)
  · grind

lemma We_finite_iff_We_eq_Ws (h : ∃ t, W e = (Ws e t).toFinset) : (W e).Finite := by
  obtain ⟨t, h⟩ := h
  rw [h]
  exact Finset.finite_toSet (Ws e t).toFinset

lemma WsNew_stabilizes_Ws_stabilizes (e : ℕ) (h : ∃ t, ∀ s ≥ t, ϕNew e s = ∅) :
    ∃ t, ∀ s ≥ t, Ws e s = Ws e t := by
  obtain ⟨t, h⟩ := h
  use t
  intro s
  have h1 : ∀ s ≥ t, Ws e s = Ws e (s-1) := by
    intro s h1
    induction s with | zero | succ s ih
    · tauto
    · apply h at h1
      simp only [add_tsub_cancel_right, Ws, append_right_eq_self, h1, empty_eq]
  induction s with | zero | succ s ih
  <;> grind
...
theorem We_finite_TFAE (e : ℕ) :
    [(W e).Finite,                          --1
      ∃ t, ∀ s ≥ t, ϕNew e s = ∅,           --2
      ∃ t, ∀ s ≥ t, Ws e s = Ws e t,      --3
      ∃ t, W e = (Ws e t).toFinset,                   --4
